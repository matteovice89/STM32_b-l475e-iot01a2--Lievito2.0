...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
  int h,t,c;
  //uint8_t Stringa[100]={0}; //per stampe a video future
  char s[5];
  ...
  while (1)
		...
		c=18;//temperatura di controllo
		h = BSP_HSENSOR_ReadHumidity();//leggo umidità
		t = BSP_TSENSOR_ReadTemp();//leggo temperatura
		t = t-3; //correzione temperatura percepita con ambientale
		...
		itoa(h, s, 10); //converti in strg int umidità
		HD44780_SetCursor(13,1);
		HD44780_PrintStr(s);//stamapa umidità display
